  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Welcome to the Paradise English School admin panel.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="text-sm font-medium text-slate-500 mb-1">Published Notices</div>
              <div className="text-4xl font-heading font-bold text-slate-900">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="text-sm font-medium text-slate-500 mb-1">Gallery Items</div>
              <div className="text-4xl font-heading font-bold text-slate-900">
      ...
      {!loading && stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <h3 className="font-bold text-slate-900 mb-4">Enquiries by Stage</h3>
            <div className="space-y-3">
              {Object.entries(stats.conversionRateByStage || {}).map(([stage, count]) => (
                <div key={stage} className="flex justify-between items-center text-sm border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                  <span className="text-slate-600 font-medium">{stage}</span>
                  <span className="font-bold text-slate-900 bg-slate-100 px-3 py-1 rounded-full">{count as number}</span>
                </div>
              ))}
              {Object.keys(stats.conversionRateByStage || {}).length === 0 && (
                <div className="text-sm text-slate-400">No enquiries found.</div>
              )}
            </div>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <h3 className="font-bold text-slate-900 mb-4">Enquiries by Branch</h3>
            <div className="space-y-3">
              {Object.entries(stats.enquiriesByBranch || {}).map(([branch, count]) => (
                <div key={branch} className="flex justify-between items-center text-sm border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                  <span className="text-slate-600 font-medium">
                    {branch === "PARADISE_ENGLISH_SCHOOL" ? "Paradise English School" : "Tender Kidz Pre-School"}
                  </span>
                  <span className="font-bold text-slate-900 bg-slate-100 px-3 py-1 rounded-full">{count as number}</span>
                </div>
              ))}
