...
export async function Footer() {
  ...
  const phoneLinks =
    settings?.phones?.map((p: string) => p.replace(/[^\d+]/g, "")) || [];
  const address =
    settings?.address || "Paradise English School, 123 Education Lane";
    ...
    <footer className="border-t bg-muted/40">
      <div className="container py-8 md:py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="md:col-span-1 flex flex-col items-start space-y-4">
            <Link href="/" className="flex items-center space-x-2">
              ...
              />
              <span className="font-heading font-bold text-school-red">
                Paradise English School
              </span>
            </Link>
            <p className="text-sm text-muted-foreground">
              Empowering students for a brighter future through quality
              education.
            </p>
          </div>
                ...
                <li className="flex flex-col">
                  ...
                  <div className="flex flex-col space-y-1 mt-1">
                    {settings.phones.map((phone: string, i: number) => (
                      <a
                        key={i}
                        href={`tel:${phoneLinks[i]}`}
                        className="hover:text-school-blue transition-colors flex items-center"
                      >
            ...
            <div className="flex space-x-4">
              ...
              {settings?.instagramUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-muted-foreground hover:text-school-red transition-colors"
                >
              ...
              {settings?.threadsUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-muted-foreground hover:text-foreground transition-colors flex items-center justify-center font-bold text-xl leading-none"
                >
        ...
        <div className="mt-8 border-t pt-8 text-center text-sm text-muted-foreground flex flex-col md:flex-row justify-between items-center">
          <p>
            © {new Date().getFullYear()} Paradise English School. All rights
            reserved.
          </p>
